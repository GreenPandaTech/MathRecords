...
* **A217058** a(12)=57 — 2 confirmed refutation path(s), lower bound found by search

**Submit A217059 first, and alone.** It is the best-evidenced term, so it is the
...
Cross-check: refuted twice along disjoint paths -- once by the primary engine and once by an earlier engine that imposes no symmetry-breaking constraint at all.
...
The refutation was re-established twice along disjoint paths -- once by the primary engine and once by an earlier engine that imposes no symmetry-breaking constraint at all.
